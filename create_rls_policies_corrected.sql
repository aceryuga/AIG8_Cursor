...
-- CORRECTED VERSION - Uses correct column names based on actual schema
...
-- Note: The property_images table may already have policies from property_images_table.sql
-- These policies will be created only if the table exists and policies don't already exist

-- Check if property_images table exists and create policies if needed
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'property_images') THEN
        ...
        DROP POLICY IF EXISTS "Users can view own property images" ON public.property_images;
        DROP POLICY IF EXISTS "Users can insert images for their own properties" ON public.property_images;
        DROP POLICY IF EXISTS "Users can update their own property images" ON public.property_images;
        DROP POLICY IF EXISTS "Users can delete their own property images" ON public.property_images;
        
        -- Create new policies with correct column references
        EXECUTE 'CREATE POLICY "Users can view own property images" ON public.property_images
            FOR SELECT USING (
                EXISTS (
                    SELECT 1 FROM public.properties 
        ...
        EXECUTE 'CREATE POLICY "Users can insert images for their own properties" ON public.property_images
            FOR INSERT WITH CHECK (
                EXISTS (
                    SELECT 1 FROM public.properties 
                    WHERE properties.id = property_images.property_id 
        ...
        EXECUTE 'CREATE POLICY "Users can update their own property images" ON public.property_images
            FOR UPDATE USING (
                EXISTS (
                    SELECT 1 FROM public.properties 
                    WHERE properties.id = property_images.property_id 
        ...
        EXECUTE 'CREATE POLICY "Users can delete their own property images" ON public.property_images
            FOR DELETE USING (
                EXISTS (
                    SELECT 1 FROM public.properties 
                    WHERE properties.id = property_images.property_id 
        ...
        RAISE NOTICE 'Property images policies created successfully';
    ELSE
        RAISE NOTICE 'Property images table does not exist, skipping policies';
    END IF;
END $$;
